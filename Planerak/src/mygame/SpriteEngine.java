...
public class SpriteEngine
{
    ...
    {
        SpriteLibrary.l_guiNode.attachChild(library.getNode());
        this.e_libraries.add(library);
    }

    public void removeLibrary(String name)
    {
        SpriteLibrary.l_guiNode.detachChildNamed(getLibrary(name).getName());
    }
            ...
            for (int s = 0; s < e_libraries.get(l).getLibrary().size(); s++)
            {
                if (!e_libraries.get(l).getStatic())
                {
                    ...
                    if (!t_sprite.getPaused() && t_sprite.getAnimated())
                    {
                        ...
                        {
                            //System.out.println(t_sprite.getFrames());
                            if (t_sprite.getCurrentFrame() < (t_sprite.getFrames() - 1))
                            {
                                //System.out.println(t_sprite.getCurrentFrame() );
                                if (t_sprite.getOnEnd().equals("Scroll"))
                                {
                                    for (int a = 0; a < t_sprite.getVertexArray().length; a++)
                                    {
                                        if (a % 2 == 0)
                                        {
                                            if (t_sprite.getVertexArray()[a] + (int) Math.pow(-1, t_sprite.getDirection()) * (1f / t_sprite.getMaxFrames()) <= 1)
                                            {
                                                t_sprite.updateVertexArray(a, t_sprite.getVertexArray()[a] + (int) Math.pow(-1, t_sprite.getDirection()) * (1f / t_sprite.getMaxFrames()));
                                            } else if (a >= 4)
                                            {
                                                t_sprite.setCurrentFrame(t_sprite.getMaxFrames());
                                            }
                                        }
                                    }
                                ...
                                } else
                                {
                                    for (int a = 0; a < t_sprite.getVertexArray().length; a++)
                                    {
                                        ...
                                        {
                                            //t_sprite.updateVertexArray(a, t_sprite.getVertexArray()[a] + (int)Math.pow(-1, t_sprite.getDirection())*(1f/t_sprite.getMaxFrames()));
                                            t_sprite.updateVertexArray(a, t_sprite.getVertexArray()[a] + t_sprite.getStride());
                                        }
                                    }
                                    t_sprite.getVertexBuffer().updateData(BufferUtils.createFloatBuffer(t_sprite.getVertexArray()));
                                    t_sprite.setTime(0);
                            ...
                            } else if (t_sprite.getOnEnd().equals("Loop"))
                            {
                                for (int a = 0; a < t_sprite.getVertexArray().length; a++)
                                {
                                    if (a % 2 == 0)
                                    {
                                        //t_sprite.updateVertexArray(a, t_sprite.getVertexArray()[a] - ((float)(t_sprite.getFrames()-1)/t_sprite.getMaxFrames()));
                                    }
                                    t_sprite.updateVertexArray(a, t_sprite.getBaseVertexArray()[a]);
                                }
                                t_sprite.getVertexBuffer().updateData(BufferUtils.createFloatBuffer(t_sprite.getVertexArray()));
                                t_sprite.setTime(0);
                            ...
                            } else if (t_sprite.getOnEnd().equals("NoLoop"))
                            {
                                for (int a = 0; a < t_sprite.getVertexArray().length; a++)
                                {
                                    if (a % 2 == 0)
                                    {
                                        t_sprite.updateVertexArray(a, t_sprite.getVertexArray()[a] - ((float) (t_sprite.getMaxFrames() - 1) / t_sprite.getMaxFrames()));
                                    }
                                }
                                t_sprite.getVertexBuffer().updateData(BufferUtils.createFloatBuffer(t_sprite.getVertexArray()));
